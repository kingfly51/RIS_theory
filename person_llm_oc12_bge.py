...
# 配置路径
outpath = 'E:/Rdaima/RIS_theory/person_llm_oc12_bge'
llm_models_dir = 'C:/workship/llm_models'

def load_embedding_model():
    """加载embedding模型"""
    model_file = os.path.join(llm_models_dir, "bge-m3")
...
def main():
    ...
    output_file = os.path.join(outpath, "编码配对相似度分析结果_bge-m3.xlsx")
    ...
    try:
        ...
        # 尝试保存为CSV
        try:
            output_df.to_csv(os.path.join(outpath, "编码配对相似度分析结果_bge-m3.csv"), index=False, encoding='utf-8-sig')
            summary_df.to_csv(os.path.join(outpath, "统计摘要_bge-m3.csv"), index=False, encoding='utf-8-sig')
            print("结果已保存为CSV文件")
